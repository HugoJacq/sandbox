...
from diags import compute_us_Kenyon1969, grad_dir
...
if False:
    ...
    def compute_Ek(ds, skip=2):
        return (
            0.5
            * (
                (
                    ds["u.x"].isel(time=slice(0, len(ds.time), skip)) ** 2
                    + ds["u.y"].isel(time=slice(0, len(ds.time), skip)) ** 2
                    + ds["u.z"].isel(time=slice(0, len(ds.time), skip)) ** 2
                )
                * dx**2
            ).sum(dim=["level", "x", "y"])
            / (L0 * L0)
    ...
    def compute_Ep(rho, z, h, Ep0, skip=2):
        Ept = (
            (rho * g * z * h)
    ...
    dz0 = H0 / nl
    z0 = np.arange(-H0 + dz0, 0, dz0)
    Tini = Ndeux / (g * betaT) * z0 + Ts
    drho0 = EOS(Tini, Ts, betaT)

    Ep0 = np.sum((1 + drho0) * g * z0 * dz0)
    Ep0 = 0
    # Ep /rho0
    Ep = compute_Ep((1 + drho), ds.z, ds.h, Ep0, skip=skip)
    Ek = compute_Ek(ds, skip=skip)
    Et = Ep + Ek
    ET0 = Et[0].values
    fig, ax = plt.subplots(1, 1, figsize=(5, 5), constrained_layout=True, dpi=100)
    ax.plot(ds.time[::skip] / Tp, 2 * Ep / ET0, c="g", label="2Ep")
    ax.plot(ds.time[::skip] / Tp, 2 * Ek / ET0, c="b", label="2Ek")
    ax.plot(ds.time[::skip] / Tp, Ep + Ek / ET0, c="k", label="Et=Ek+Ep")
    ax.set_xlabel("t/Tp")
    ax.set_ylabel("E")
...
if True:
    ...
    # nice slope
    x0, x1 = 1e2, 3e2  # adjust to sit in an empty-ish part of the plot
    slope = -2.5
    y0 = 1e-3  # adjust vertically so the line sits near your data
    y1 = y0 * (x1 / x0) ** slope
    ax.loglog([x0, x1], [y0, y1], "k--", lw=1)
